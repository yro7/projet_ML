...
\begin{center}
	\begin{minipage}{13cm}
		...
		Notre étude personnelle (Partie V) constitue un benchmark exhaustif de \textbf{35\,669 expériences} croisant 9 méthodes de prétraitement, 10 classifiers et 3 méthodes d'évaluation. Les résultats montrent que la combinaison \textbf{MFCC Summary × Régression Logistique} atteint les meilleures performances avec une accuracy moyenne de 96.2\% en validation croisée.
	\end{minipage}
\end{center}
...
\midrule
\textbf{Total} & \textbf{35\,669} & expériences (avec grilles d'hyperparamètres) \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
1 & \textbf{Random Forest} & 2\,833 & \textbf{0.655} & 1.000 \\
...
8 & Logistic Regression & 1\,429 & 0.569 & 1.000 \\
...
MFCC Summary $\times$ Neural Network & 0.909 & 1.000 & 145 \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
    ...
    \item \textbf{782 erreurs} (sur 35\,669 runs) — essentiellement des \texttt{ValueError} dans les combinaisons Wavelet $\times$ AdaBoost dues à des conflits de buffers.
\end{itemize}
...
\midrule
Régression Logistique & 96.30\% & \textbf{98.15\%} & 96.30\% & \textbf{90.74\%} \\
SVM (RBF) & 96.30\% & 96.30\% & 92.59\% & 85.19\% \\
Random Forest & 94.44\% & 96.30\% & 88.89\% & 83.33\% \\
Bagging Tree & 87.04\% & 88.89\% & 90.74\% & 83.33\% \\
AdaBoost & 79.63\% & 83.33\% & 88.89\% & 79.63\% \\
Réseau de neurones (MLP) & 92.59\% & 96.30\% & 94.44\% & 88.89\% \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{figure}[H]
    ...
    \caption{Évolution de l'accuracy (LOO CV) des classifieurs sur les MFCC Summary en fonction du niveau de bruit (SNR) injecté dans le signal audio brut.}
    \label{fig:noise_robustness}
\end{figure}

\textbf{Analyse des performances et résilience :}
\begin{itemize}
    \item \textbf{La Régression Logistique est extrêmement robuste} : Elle conserve une précision de 90.74\% même sous un bruit intense (0\,dB). Sa nature linéaire limite le surapprentissage lié aux fluctuations aléatoires induites par le bruit. Les coefficients cepstraux moyennés dans le temps filtrent une grande partie du bruit additif blanc, permettant au classifieur linéaire de conserver sa frontière de décision.
    \item \textbf{Effet de régularisation par le bruit} : Étonnamment, pour presque tous les modèles (LR, RF, Bagging, AdaBoost, MLP), l'ajout d'un bruit léger (20\,dB) améliore légèrement ou maintient les scores de généralisation. Ce phénomène classique est assimilable à de l'augmentation de données (\textit{noise injection as regularization}) qui empêche les classifieurs d'ajuster trop précisément les frontières sur le très petit échantillon de 54 exemples.
    \item \textbf{Sensibilité relative du SVM et des arbres} : Le SVM RBF et la Random Forest montrent des baisses plus marquées à partir de 10\,dB (jusqu'à 85.19\% et 83.33\% respectivement à 0\,dB). Les frontières non linéaires complexes du SVM RBF et les décisions rigides par seuil des arbres de décision sont plus sensibles à la gigue des features causée par le bruit.
    \item \textbf{Stabilité du MLP} : Le réseau de neurones multicouche maintient une excellente résilience (92.59\% $\rightarrow$ 96.30\% $\rightarrow$ 94.44\% $\rightarrow$ 88.89\%), démontrant la robustesse des représentations apprises par ses couches cachées.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Le preprocessing est roi.} Les MFCC Summary (combinant moyenne et écart-type des coefficients cepstraux) dominent systématiquement toutes les représentations spectrales alternatives (FFT+PCA, STFT, Wavelet). La représentation perceptuelle de Mel est naturellement adaptée à la discrimination de parole.
...
\textbf{Perspectives :} L'extension à davantage de classes (13 commandes disponibles), l'ajout de données personnelles, et l'utilisation de techniques d'augmentation de données audio (ajout de bruit, time stretching) permettraient de tester ces méthodes dans des conditions plus réalistes.
